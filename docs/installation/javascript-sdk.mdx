...
Vous pouvez explorer dans les Tabs suivantes toutes les possibilités d'affichage que propose l'objet resource. 
...
![Marianne synthese](/img/synthese.png)
...
#### Permettre l'ajout de commentaires lorsqu'aucun débat n'est lié à l'article (optionnel)
...
![Widget synthese](/img/synthese_widget.png)
...
Vote permet d'épingler un débat ou d'afficher le dernier créer. 
 ...
 resource: {
            name: "group",
            id: "DEBATE_IDENTIFIER" // L'ID du débat qu'on souhaite afficher.
        }
```
...
L'ID correspond ici à celui du débat. Group permet d'afficher un débat en particulier via son ID. 
 ...
 resource: {
            name: "consultation",
            id: "CONSULTATION_IDENTIFIER" // L'ID de la consultation qu'on souhaite afficher.
        }
```
...
L'ID correspond ici à celui de la consultation. Group permet d'afficher une consultation en particulier via son ID. 
 ...
 <TabItem value="Arguments" label="Arguments">
   resource: {
            name: "argument" OU "proposal",
            id: "OBJECT_IDENTIFIER" // L'ID de l'argument ou de la proposition
        }
```
