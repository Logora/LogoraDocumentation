...
<Tabs>
  <TabItem value="Synthese" label="Synthese" default>
    La valeur synthese permet d'afficher une question de débat lorsqu'un débat est lié à l'article. Si aucun débat n'est lié à l'article, vous pouvez décider d'afficher des commentaires à la place, en autorisant l'ajout dans commentaires dans la configuration de l'espace d'administration. C'est la valeur par défaut et ce qui est utilisé dans la très grande majorité des cas par nos partenaires. 
